/* calc.js
   Pure calculation functions mirroring the formulas found in the original
   workbook (1.profile, 2.revenues, 3.costs, 4.labour,
   5.household expenditures, 6.results). Kept separate from rendering code
   so the logic can be checked against the source formulas independently.
   Every function takes plain data and returns plain numbers — no DOM, no
   side effects.
*/
...
const COCOA_SHARE_WEIGHT = {};
OPTIONS.cocoa_share.forEach(o => { COCOA_SHARE_WEIGHT[o.key] = o.weight; });

/* weighted cocoa allocation, mirrors the SUMIF-with-wildcards formula
   used throughout 3.costs (e.g. E15, H15, E33, E48, E63) */
function cocoaWeightedSum(rows, valuePick, sharePick) {
  return rows.reduce((s, row) => {
    const w = COCOA_SHARE_WEIGHT[sharePick(row)] || 0;
...
function calcProfile(profile) {
  ...
  const cocoaAreaSharecropped = sum(
    plots.filter(p => (p.landArrangement || "").includes("sharecropper")),
    p => (["cocoa", "cocoa_intercropped"].includes(p.crop) ? p.area : 0)
  );
  const pctCocoaSharecropped = totalCocoaArea ? cocoaAreaSharecropped / totalCocoaArea : 0;

  const farmSharecropped = sum(plots.filter(p => (p.landArrangement || "").includes("sharecropper")), p => p.area);
  ...
  const workingAge = [...working, ...notWorking].filter(r => r.age !== "" && r.age != null && num(r.age) >= 18 && num(r.age) < 65).length;
...
/* ---------- 2.revenues ---------- */
function calcMonthlySales(rows) {
  // rows: [{month, volume, price}], revenue = volume * price
...
function calcOtherIncomeRows(rows) {
  return sum(rows, r => r.amount);
}

function calcCashCropRow(row) {
...
function calcCashCrops(rows) {
  return sum(rows, r => calcCashCropRow(r).revenue);
}

function calcStapleRow(row) {
...
function calcSharecropPayments(rows) {
...
function calcLabour(rows) {
  ...
  const rowLabourCost = r => num(r.hiredDays) * num(r.dailyWage) + num(r.serviceCost);
...
function calcExpenditureCategory(row) {
  return num(row.q1) + num(row.q2) + num(row.q3) + num(row.q4);
}
function calcExpenditures(exp) {
  const food = calcExpenditureCategory(exp.food || {});
  const education = calcExpenditureCategory(exp.education || {});
  const healthcare = calcExpenditureCategory(exp.healthcare || {});
  const other = calcExpenditureCategory(exp.other || {});
  return { food, education, healthcare, other, total: food + education + healthcare + other };
}

/* ---------- 6.results ---------- */
function calcResults(record) {
  const profile = calcProfile(record.profile || {});
  const revenues = calcRevenues(record.revenues || {});
  const costs = calcCosts(record.costs || {});
  const labour = calcLabour(record.labour || []);
  const expenditures = calcExpenditures(record.expenditures || {});
