/* data-model.js
   Defines the shape of a household record and the schema for every
   repeating table in the tool. app.js reads these schemas to render
   generic add/remove-row tables instead of hand-coding each one. */

function emptyRow(columns) {
  const row = {};
  columns.forEach(c => { row[c.key] = c.type === "number" ? "" : ""; });
  return row;
}
...
function makeEmptyTable(schemaKey) {
